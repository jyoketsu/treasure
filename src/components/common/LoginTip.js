...
class LoginTip extends Component {
    ...
    render() {
        ...
        const isWebView = util.common.getSearchParamValue(location.search, 'web-view');
        return (
            <div className="login-tip" style={{ display: isWebView ? 'none' : 'flex' }}>
                {
                    logoSize ?
                        <i className={`menu-logo`} style={{
                ...
                <Link to={`/${stationDomain}`}>{nowStation ? nowStation.name : ''}</Link>
                <Button type="primary" onClick={() => history.push('/account/login')}>登录</Button>
            </div>
        );
    }
