...
class Space extends Component {
  render() {
    return <div className="space" style={{ width: this.props.width }}></div>;
  }
}
...
class Link extends Component {
  render() {
    ...
    return (
      <span className="my-link" style={style} onClick={handleClick}>
        {text}
      </span>
    );
  }
...
class Select extends Component {
  ...
  render() {
    let icon = this.props.icon ? (
      <i className={`select-icon ${this.props.icon}`}></i>
    ) : null;
    const selectOptions = this.props.options;
    let options = (
      <ul className="select-options">
        {selectOptions.map((option, index) => (
          <li key={index} onClick={this.handleOptionClick.bind(this, option)}>
            {option.text}
          </li>
        ))}
      </ul>
  ...
  render() {
    ...
    return (
      <ClickOutside onClickOutside={this.collapseOptions}>
        <div
          className={`my-select dropdown-button ${this.props.className || ""}`}
          style={this.props.style}
          onClick={this.switchOptions}
        >
          <input
          ...
          textInput = (
            <div className={className} style={this.props.style}>
              ...
              <DatePicker
                placeholder={this.props.placeholder}
                defaultValue={
                  this.props.value ? moment(this.props.value, "YYYY-MM-DD") : ""
                }
...
class FileUpload extends Component {
  constructor(props) {
    ...
    switch (this.props.metaType) {
      case "image":
        this.mimeType = ["image/png", "image/jpeg", "image/svg+xml"];
        this.accept = "image/*";
        break;
      case "video":
        this.mimeType = ["video/mp4", "video/ogg"];
        this.accept = "video/*";
        break;
      default:
        this.mimeType = ["image/png", "image/jpeg", "image/svg+xml"];
  ...
  handleFileChange(event) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      let observable = qiniu.upload(
        file,
        `${util.common.guid(8, 16)}${file.name.substr(
  ...
  render() {
    ...
    return (
      <i
        className={`file-upload-button ${this.props.className}`}
        style={this.props.style}
      >
        {fileInput}
        {this.props.text}
